use core::{panic::PanicInfo, sync::atomic::AtomicIsize};
...
use crate::{
    mm::layout::{__bss_end, __data_end, __text_end, __text_start},
    task,
};

static PANIC_HART: AtomicIsize = AtomicIsize::new(-1);
...
fn panic(info: &PanicInfo) -> ! {
    ...
    } else if let Some(task) = cur_task {
        error!(
